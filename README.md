...
- **Click a country** for hints — an unguessed one offers its first letters and
  its capital city; a guessed one shows its name and capital outright.
...
- Progress is not saved; reloading starts a new game.
...
- [ ] **Commit the geojson build script** — the country data was generated by a
      script that was never committed, so editing aliases means re-deriving it.
...
- [ ] Add a `LICENSE`.
- [ ] Enter-to-submit, skipping the 400 ms debounce.
